    ...
    try:
        ...
        for chunk in discourse_chunks:
            try:
                ...
                if not isinstance(embedding, list):  # Validate embedding format
                ...
                if similarity >= SIMILARITY_THRESHOLD:
                    ...
                    if not url.startswith("http"):
                        url = f"https://discourse.onlinedegree.iitm.ac.in/t/{url}"  # Fix missing protocol
        ...
        for result in results:
            key = f"{result['source']}_{result['id']}"
        ...
        raise HTTPException(status_code=500, detail="Error finding similar content")
...
async def generate_answer(question, relevant_results, max_retries=2):
    ...
    while retries < max_retries:
        try:
            ...
            if len(context) > 4000:  # Adjust based on model token limit
            ...
            Answer: [Your answer]
            Sources: [List of URLs and snippets]
            """
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, json=payload) as response:
                    if response.status == 200:
                        ...
                        raise HTTPException(status_code=response.status, detail="Error generating answer")
        except Exception as e:
            retries += 1
            if retries >= max_retries:
...
async def process_multimodal_query(question, image_base64):
    ...
    try:
        ...
        payload = {
            "model": "gpt-4o-mini",
            "messages": [
                {"role": "user", "content": f"Describe this image and answer the question: {question}"},
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=payload) as response:
                if response.status == 200:
                    ...
                    return await get_embedding(question)  # Fallback to text-only query
    except Exception as e:
        return await get_embedding(question)  # Fallback to text-only query
